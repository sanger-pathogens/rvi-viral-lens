...
def parse_index_label_map(path):
    """SEQIDX_<n> -> species, from call_themisto_species.py's out-index-label-map (one
    most-hit reference index chosen per called species, so this is a strict 1:1 mapping)."""
    mapping = {}
    with open(path) as fh:
        for line in fh:
            record_id, species = line.rstrip("\n").split("\t")
            mapping[record_id] = species
    return mapping


def parse_species_hits(path):
    """species -> hit_count, from call_themisto_species.py's out-species-hits TSV."""
    hits = {}
    with open(path) as fh:
        next(fh)  # header
        for line in fh:
...
def main():
    parser = argparse.ArgumentParser(
        description="Build the per-species Themisto-map QC table: one row per called "
                    "species, from its single most-hit mapped reference sequence."
    )
    ...
    parser.add_argument("--index-label-map", required=True, help="SEQIDX_<n> -> species TSV")
    ...
    record_id_to_species = parse_index_label_map(args.index_label_map)
    species_hits = parse_species_hits(args.species_hits)

    with open(args.out, "w") as out:
        out.write(
            "sample_id\tspecies\thit_count\treference_record\treference_length\tquery_length\t"
            "covered_bases\tbreadth_pct\tmean_depth\tmeanbaseq\tmeanmapq\treads_mapped\n"
        )
        for record_id, species in record_id_to_species.items():
            ...
            out.write(
                f"{args.sample_id}\t{species}\t{species_hits.get(species, '')}\t{record_id}\t{reference_length}\t"
